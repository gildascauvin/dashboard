...
@Component({
  selector: 'app-athlete-programs-detail',
  templateUrl: './athlete-programs-detail.component.html',
  styleUrls: ['./athlete-programs-detail.component.scss']
})
export class AthleteProgramsDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.id = this.currentProgram.program_id;
    ...
    this.sub.templatesReset = this.templatesService.onTemplateReset.subscribe(() => {
      this._initTemplate();
    });
  ...
  ngOnDestroy() {
  	...
  	this.sub.model && this.sub.model.unsubscribe();
    this.sub.updateUsers && this.sub.updateUsers.unsubscribe();
    this.sub.templates && this.sub.templates.unsubscribe();
    this.sub.templatesAutosave && this.sub.templatesAutosave.unsubscribe();
    this.sub.templatesReset && this.sub.templatesReset.unsubscribe();
  ...
  private _initTemplate() {
    this.sub.model = this.usersService.getProgram(this.id).subscribe((templateData: any) => {
      if (templateData.program_id) {
        this.model = templateData;
        this.weeks = templateData.program || [];
  ...
  autoSave() {
    let model: any = {
      ...
      program_json: JSON.stringify(this.weeks)
    }

    model.days_count = 0;
    model.weeks_count = this.weeks.length;
    model.workouts_count = 0;

    _.forEach(this.weeks, (week) => {
      _.forEach(week, (day) => {
        if (!day.restDay.isRestDay || day.workouts.length) {
          model.days_count++;
        }

        model.workouts_count += day.workouts.length;
      });
    });

    if (!this.model.program_id) {
      return;
    }
    this.sub.templatesAutosave && this.sub.templatesAutosave.unsubscribe();

    this.sub.templatesAutosave = this.usersService.updateProgram(model).subscribe((data: any) => {
      if (!data.errors) {
  ...
  addWorkoutToWeek(day, i, y) {
    ...
    this.addExerciceToWorkout(day, day.workouts[day.workouts.length - 1].exercices, day.workouts[day.workouts.length - 1]);
  }
